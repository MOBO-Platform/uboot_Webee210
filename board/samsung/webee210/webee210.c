...
#define GPJ2CON_1   (*( volatile unsigned int  *)0xE0200280)
#define GPJ2DAT_1   (*( volatile unsigned char *)0xE0200284)

#define LED_A_ON      (1<<0)
#define LED_A_OFF    ~(1<<0)
#define LED_B_ON      (1<<1)
#define LED_B_OFF    ~(1<<1)
#define LED_C_ON      (1<<2)
#define LED_C_OFF    ~(1<<2)
#define LED_D_ON      (1<<3)
#define LED_D_OFF    ~(1<<3)
...
void show_led_progress (int status)
{
   static char state = 0;
   if (state == 0) {
	   GPJ2CON_1 &= 0xffff0000;
       GPJ2CON_1 |= 0x00001111;
       state = 1;
   }
...
#define GPA1CON     *(( volatile unsigned short *)0xE0200020)
